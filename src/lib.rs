...
fn lemire<F: Float>(neg: bool, mut man: u64, exp10: i32) -> Option<F> {
    ...
    let (hi, lo) = {
        ...
        if hi & F::MASK == F::MASK && lo.overflowing_add(man).1 {
            let (hi2, lo2) = mul128(man, POWERS_OF_TEN[(exp10 - POWERS_OF_TEN_MIN) as usize].0);
            let (merged_lo, overflowed) = lo.overflowing_add(hi2);
            lo = merged_lo;
            if overflowed {
                hi += 1;
            }
            if hi & F::MASK == F::MASK && lo == u64::MAX && lo2.overflowing_add(man).1 {
                return None;
            }
        }
        (hi, lo)
...
const POWERS_OF_TEN_MIN: i32 = -348;
const POWERS_OF_TEN_MAX: i32 = 347;
// log2(10) ≒ 217706 / 2^16
const LOG2_10: (i32, i32) = (217706, 16);
const POWERS_OF_TEN: [(u64, u64); 696] = [
